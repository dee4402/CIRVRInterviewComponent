...
namespace RealisticEyeMovements {
	...
	public class EyeAndHeadAnimatorEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			DrawHeadControl(eyeAndHeadAnimator);
			DrawControlData(eyeAndHeadAnimator);
				...
				if (GUILayout.Button("Import"))
				{
					...
					if (false == string.IsNullOrEmpty(filename))
					{
						...
						else if ( System.IO.Path.GetFileName(filename).StartsWith("MCS_") )
							EditorUtility.DisplayDialog("Cannot import", "For Morph3D models, you need to uncomment '#define USE_MCS' at the top of Scripts/ControlData.cs and Editor/EyeAndHeadAnimatorEditor.cs as decribed in the manual.", "Ok");
						else
		...
		void DrawControlData(EyeAndHeadAnimator eyeAndHeadAnimator)
		{
			...
			bool noAnimator = isEyeBoneControl && animator == null;
			...
			//*** Error message if any data for eye control is missing
			{
				...
				if ( isEyeBoneControl || isEyeballControl )
				{
					...
					if ( noAnimator && isEyeBoneControl )
					{
			...
			bool isEyelidBlendshapeControl = eyelidControl == ControlData.EyelidControl.Blendshapes || eyelidControl == ControlData.EyelidControl.MCS_Blendshapes	;
			...
			else
			{
				...
				controlData.eyeWidenOrSquint = EditorGUILayout.Slider(new GUIContent("Eye widen or squint", tooltip), controlData.eyeWidenOrSquint, -1, 0);
			}
		}



		void DrawHeadControl(EyeAndHeadAnimator eyeAndHeadAnimator)
		{
			Animator _animator = eyeAndHeadAnimator.GetComponent<Animator>();
			if ( _animator == null || _animator.GetBoneTransform(HumanBodyBones.Head) == null )
			{
				const string tooltipHeadXform = "If you have a non-Mecanim character, assign the head bone or head gameObject here.";
				eyeAndHeadAnimator.headBoneNonMecanimXform = EditorGUILayout.ObjectField(new GUIContent( "Head transform", tooltipHeadXform), eyeAndHeadAnimator.headBoneNonMecanimXform, typeof(Transform), true ) as Transform;
			}

			const string tooltipHeadWeight = "How much this component controls head direction.";
			eyeAndHeadAnimator.headWeight = EditorGUILayout.Slider(new GUIContent("Head weight", tooltipHeadWeight), eyeAndHeadAnimator.headWeight, 0, 1);

			const string tooltipHeadSpeed = "Increases or decreases head movement speed (1: normal)";
			eyeAndHeadAnimator.headSpeedModifier = EditorGUILayout.Slider(new GUIContent("Head speed modifier", tooltipHeadSpeed), eyeAndHeadAnimator.headSpeedModifier, 0.1f, 5);
		}
